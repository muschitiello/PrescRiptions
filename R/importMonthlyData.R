...
importMonthlyData = function(yyyy = 2019, mm = NULL){
  ...
  out = list(get(paste0("plpd",prefix)),
             get(paste0("chem",prefix)),
             get(paste0("addr",prefix)))

  names(out) = c(paste0("plpd",prefix),paste0("chem",prefix),paste0("addr",prefix))
