...
monthlyData_import = function(settings,whichData = "all",sample=TRUE){
  ...
  if(plpdTF){
    ...
    if(dir.exists(paste0(dirs$plpdRootDir,folder)) & length(list.files(paste0(dirs$plpdRootDir,folder)))==3){
      ...
      #### if not, download it and put it in the workspace
      ...
      for(j in plpdFiles){
        ...
        if(grepl("ADDR|addr",j)){
          assign(paste0("addr_",prefix),data.table(read.csv(paste0(td,"/",j),stringsAsFactors = FALSE)))
        }
        if(grepl("CHEM|chem",j)){
          assign(paste0("chem_",prefix),data.table(read.csv(paste0(td,"/",j),stringsAsFactors = FALSE)))
